...
SELECT 
    t.city,
    t.city_total_spent,
    ROUND((t.city_total_spent / tc.spent_total), 2) * 100 AS total_perc_city
FROM
    total t
        JOIN
...
SELECT 
    ...
    ROUND(SUM(cc.amount) / (SELECT 
                    SUM(ct.amount)
                FROM
                    credit_card_transaction ct) * 100,
            2) AS city_total
FROM
...
SELECT 
    ...
    ROUND(SUM(cc.amount) / SUM(ct.amount) * 100, 2) AS city_total
FROM
    credit_card_transaction cc
        INNER JOIN
